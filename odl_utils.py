...
def parser_ConeBeamGeometry(
    ...
    # Normalize axial positions so reconstruction z starts at 0.
    # Removed the previous hard-coded detector-height offset.
    axial_positions = axial_positions.astype(np.float32).copy()
    axial_positions -= axial_positions[0]

    REC_MIN_X = metadata['REC_MIN_X']
    REC_MIN_Y = metadata['REC_MIN_Y']
    REC_MIN_Z = float(axial_positions[0])

    REC_MAX_X = metadata['REC_MAX_X']
    REC_MAX_Y = metadata['REC_MAX_Y']
    REC_MAX_Z = float(axial_positions[-1])

    REC_NPX_X = metadata['REC_NPX_X']
    REC_NPX_Y = metadata['REC_NPX_Y']
    REC_NPX_Z = int(round((REC_MAX_Z - REC_MIN_Z) / metadata['REC_PIC_SIZE'])) + 1
    ...
    # Debug prints for geometry verification
    print("[GEOM] detector_partition.shape:", detector_partition.shape, flush=True)
    print("[GEOM] reco_space.shape:", reco_space.shape, flush=True)
    print("[GEOM] reco_space.min_pt:", reco_space.min_pt, flush=True)
    print("[GEOM] reco_space.max_pt:", reco_space.max_pt, flush=True)
    print("[GEOM] reco_space.cell_sides:", reco_space.cell_sides, flush=True)
    print("[GEOM] computed pitch:", PITCH, flush=True)
