# Script filename: odl_utils_v3.py
...
def compute_local_pitch_mm_per_turn(angles: np.ndarray, axial_positions: np.ndarray) -> float:
...
def build_global_helical_geometry_v3(
    ...
    """
    Global helical geometry prototype.

    Important:
    - uses full angles and axial_positions
    - uses one global reconstruction space
    - currently uses pitch estimated from angles+axial_positions
    - does NOT use slab fusion
    - does NOT yet use measured shifts.npy
    """
    ...
    pitch_mm_per_turn = compute_local_pitch_mm_per_turn(angles, axial_positions)
    ...
    if use_curved_detector:
        det_curv = (det_curvature_radius, None)
    else:
        det_curv = None
    ...
    debug_info = {
        ...
        "mode": "global_helical_geometry_v3",
    }
